...
@section('title', $group->name . ' - Grup Düzenle')
@section('page-title', 'Grup Düzenle')
    ...
    <div class="col-xl-10">
        <div class="table-card">
            <div class="table-header d-flex justify-content-between align-items-center">
                <div><i class="fas fa-pen-to-square me-2"></i>{{ $group->name }} - Düzenle</div>
            ...
            <div class="p-4">
                ...
                <form id="groupEditForm" action="{{ route('admin.groups.update', $group->id) }}" method="POST">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="workshop_id" required>
                                @foreach($workshops as $workshop)
                                    <option value="{{ $workshop->id }}" {{ (int) old('workshop_id', $group->workshop_id) === $workshop->id ? 'selected' : '' }}>{{ $workshop->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="instructor_id" required>
                                @foreach($instructors as $instructor)
                                    <option value="{{ $instructor->id }}" {{ (int) old('instructor_id', $group->instructor_id) === $instructor->id ? 'selected' : '' }}>{{ $instructor->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Kontenjan</label>
                            <input type="number" class="form-control" name="capacity" value="{{ old('capacity', $group->capacity) }}" min="1" required>
                        </div>
                    </div>

                    @php($selectedDays = old('day_of_weeks', $group->day_of_weeks ?? [$group->day_of_week]))
                    ...
                    <div id="day-schedule-container" class="row g-3">
                        @foreach($dayMap as $dayKey => $dayLabel)
                            @php($slot = old("day_schedules.$dayKey", ($group->day_schedules[$dayKey] ?? null)))
                                ...
                                <div class="border rounded p-3">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-6">
                                            ...
                                            <input type="time" class="form-control" name="day_schedules[{{ $dayKey }}][start]" value="{{ $slot['start'] ?? optional($group->start_time)->format('H:i') }}">
                                        </div>
                                        <div class="col-6">
                                            <label class="form-label">Bitiş</label>
                                            <input type="time" class="form-control" name="day_schedules[{{ $dayKey }}][end]" value="{{ $slot['end'] ?? optional($group->end_time)->format('H:i') }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row g-3 mt-2">
                        <div class="col-md-6">
                            ...
                            <input type="date" class="form-control" name="group_start_date" value="{{ old('group_start_date', optional($group->group_start_date)->format('Y-m-d')) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Grup Bitiş Tarihi</label>
                            <input type="date" class="form-control" name="group_end_date" value="{{ old('group_end_date', optional($group->group_end_date)->format('Y-m-d')) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Durum</label>
                            <select class="form-select" name="status">
                                <option value="active" {{ old('status', $group->status) === 'active' ? 'selected' : '' }}>Aktif</option>
                                <option value="inactive" {{ old('status', $group->status) === 'inactive' ? 'selected' : '' }}>Pasif</option>
                                <option value="full" {{ old('status', $group->status) === 'full' ? 'selected' : '' }}>Dolu</option>
                            </select>
                        </div>
                        <div class="col-md-12">
...
<script>
const formEl = document.getElementById('groupEditForm');
